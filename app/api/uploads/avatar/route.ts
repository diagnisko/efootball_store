...
import { createAvatarUploadPost, StorageNotConfiguredError, uploadAvatar } from "@/lib/storage";
...
export async function POST(req: Request) {
  ...
  if (req.headers.get("content-type")?.includes("multipart/form-data")) {
    ...
    try {
      return NextResponse.json(await uploadAvatar(userId, file.name, file.type, new Uint8Array(await file.arrayBuffer())));
    } catch (e) {
  ...
  try {
    const result = await createAvatarUploadPost(userId, fileName, contentType);
